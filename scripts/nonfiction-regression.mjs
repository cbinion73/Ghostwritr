...
const BOOK_SLUG = process.env.GHOSTWRITR_BOOK_SLUG ?? "nonfiction-smoke";
const OUTPUT_DIR = path.resolve("test-results/nonfiction-regression");
...
async function expectBodyContains(page, values) {
  ...
  return {
    ...
    missing: values.filter((value) => !body.includes(value)),
  };
}
...
async function main() {
  ...
  const flows = [
    {
      key: "promise",
      url: `${BASE_URL}/books/${BOOK_SLUG}/promise`,
      expected: ["Promise", "Audience", "Truth", "Transformation", "Market", "Recommendations"],
    },
    {
      key: "outline",
      url: `${BASE_URL}/books/${BOOK_SLUG}/outline`,
      expected: ["Outline", "Sections", "Chapters"],
    },
    {
      key: "dashboard",
      url: `${BASE_URL}/books/${BOOK_SLUG}/dashboard`,
      expected: ["Parallel Progress", "Open Chapter Draft", "Open Publish", "Publish Handoff"],
      expectedAny: [["Status: Synced", "Status: Refresh required"]],
    },
    {
      key: "chapter-draft",
      url: `${BASE_URL}/books/${BOOK_SLUG}/chapter-draft`,
      expected: ["Chapter Draft", "Draft Quality", "QUALITY SIGNALS", "Source Context"],
    },
    {
      key: "editing",
    ...
    },
    {
      ...
      expected: ["Publish", "Publish Package Sync", "Publishing Package", "Editorial Gate", "Draft Quality Baseline", "Front Matter Plan", "Back Matter Plan", "Typesetting Plan", "Trim profile", "Signature plan", "Section starts on recto", "Estimated total pages", "Preflight Checks", "Finalize Handoff", "Production Deliverables", "Export Interior Layout", "Recommendation: blocked"],
    },
  ];
  ...
  const exportAssertions = exportChecks.map((check) => {
    ...
    const expected =
      check.format === "html"
        ? ["Front Matter", "Back Matter", "Table of contents", "Acknowledgments"]
        : check.format === "json"
          ? ['"frontMatter"', '"backMatter"', '"trimSize"']
        : check.format === "publish-package"
            ? ["publish-package.json", "layout-manifest.json", "cover-brief.json", "distribution-manifest.json", ".html"]
            : ["interior.html", "print.css", "typeset-package.json", "layout-manifest.json", "cover-brief.json"];
    return {
      format: check.format,
      status: check.status,
      missingExpected: expected.filter((value) => !normalized.includes(value)),
    };
  });
