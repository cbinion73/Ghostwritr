      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10 }}>
        {phases.map((phase) => {
          ...
          return phase.enabled ? (
            <Link
              key={phase.id}
              href={`/books/${input.slug}/outline?phase=${phase.id}`}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 10,
                  ...
                  <details className="dossier-packet" key={chapter.id}>
                        ...
                        <div className="dossier-packet-body">
                          <div className="metric-row" style={{ marginBottom: 12 }}>
                            ...
                            <Link className="btn" href={`/books/${slug}/outline?phase=chapter-breakdowns&targetType=chapter&targetId=${chapter.id}`}>
                              Open Phase 2 View
                            </Link>
                          </div>
    ...
    <div className="page-shell">
      <aside className="glass-panel sidebar">
        ...
        <div className="muted" style={{ marginBottom: 20 }}>
          <div>
            Book: <strong>{outlineWorkspace.book.titleWorking ?? "Untitled Book"}</strong>
          </div>
          <div style={{ marginTop: 6 }}>
            Outline stage: <strong>{outlineWorkspace.outlineStage?.status ?? "NOT_STARTED"}</strong>
          </div>
          <div style={{ marginTop: 6 }}>
            Book Pitch: <strong>{outlineWorkspace.bookPromiseReport ? "LOCKED INPUT" : "MISSING"}</strong>
          </div>
        </div>
      ...
      <main className="main-column">
        <section className="glass-panel topbar">
          ...
          <div className="button-row">
            ...
            {currentPhase === "chapter-breakdowns" ? (
              <>
                <Link className="btn" href={`/books/${slug}/outline?phase=sections-chapters`}>
                  Back to Phase 1
                </Link>
            ...
            {currentPhase === "full-toc" ? (
              <>
                <Link className="btn" href={`/books/${slug}/outline?phase=chapter-breakdowns`}>
                  Back to Phase 2
                </Link>
                  ...
                  <div className="toc-list">
                    {latestOutline.sections.map((section) => (
                      <article className="toc-entry" key={section.id}>
                        <Link
                          href={`/books/${slug}/outline?phase=sections-chapters&targetType=section&targetId=${section.id}`}
                          className="toc-link"
                        >
                        ...
                        <div className="toc-sublist">
                          {section.chapters.map((chapter) => (
                            <div className="toc-subentry" key={chapter.id}>
                              <Link
                                href={`/books/${slug}/outline?phase=sections-chapters&targetType=chapter&targetId=${chapter.id}`}
                        ...
                        <div className="toc-sublist">
                          {section.chapters.map((chapter) => (
                            <div className="toc-subentry" key={chapter.chapterId}>
                              <Link
                                href={`/books/${slug}/outline?phase=chapter-breakdowns&targetType=chapter&targetId=${chapter.chapterId}`}
                                className="toc-section-link"
                              >
                                <div className="toc-subtitle">
                              ...
                              <div className="toc-subchapter-list">
                                {chapter.paragraphs.map((paragraph) => (
                                  <Link
                                    key={paragraph.id}
                                    href={`/books/${slug}/outline?phase=chapter-breakdowns&targetType=paragraph&targetId=${paragraph.id}`}
